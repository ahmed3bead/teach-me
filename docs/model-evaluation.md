...
> **Blocked for this remediation:** request-shaping sources changed, so the checked-in pricing policy is intentionally stale and release preflight must reject it before dispatch. The command below is a non-runnable template. A separate reviewed change must recalculate the complete source-bound ceiling, update every cost-sensitive hash, and obtain new authorization before any paid request. Do not reuse the previous `$18.912125` authorization.
...
python3 scripts/run_behavioral_evals.py \
  ...
  --pass-threshold 0.90 \
  --protocol-retries 0 \
  --pricing-file REGENERATED_SOURCE_BOUND_POLICY.json \
  --authorized-cost-usd RECALCULATED_AUTHORIZATION \
  --conservative-max-cost-usd RECALCULATED_CEILING \
...
The response adapter receives JSON with `type: generate`, chronological history, explicit locale, case identifiers, and an immutable `prompt_packet`. The packet always contains the complete content and SHA-256 of `SKILL.md`, `core-teaching-policy.md`, and `teaching-contract.md`, plus every conditionally routed reference, routing axes, an explicit capability-state manifest, and controlled synthetic inputs resolved from stable fixture IDs. For PDF routing with executable local rendering, the response contract requires exactly one self-contained printable HTML artifact; the adapter validates that source and derives the PDF, while continuing to reject zero or multiple HTML sources. It returns complete invocation evidence and a structured artifact array (empty when no file is requested):
...
Every grader `PASS` quote must occur exactly in its identified assistant turn or generated artifact. The grader must copy response and non-HTML evidence verbatim without whitespace or diacritic normalization and must never cite learner text as response evidence. For HTML only, validation compares one contiguous learner-visible text span in reading order after parsing markup and excluding `script` and `style` content; markup may be omitted but visible words may not be spliced or paraphrased. Non-HTML evidence remains an exact raw substring. If any verdict cites absent text or otherwise violates the grader protocol after the configured retry count, none of that grader's verdicts are accepted: the case fails, all criteria receive fail-closed protocol evidence, and the run continues. The report retains the invalid verdicts and evidence, their hash, model identity, settings, timing, request/API metadata, usage, and protocol error. With `--protocol-retries 0`, the grader is not retried. Contradictory-`PASS` downgrade remains separate: it converts the individual verdict conservatively and increments the adjustment count without making an additional request.
