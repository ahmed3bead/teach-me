...
The adapter uses Standard processing (`service_tier=default`), `store=false`, strict JSON Schema output, no tools, no SDK retries, controlled fixtures only, `reasoning.effort=none`, and `temperature=0`. Ordinary response and all grader requests have a 2,048-token output bound. Only response payloads whose committed routing requests a `curriculum` or `learning-pack` and whose file capability is `executable_temp` receive the explicit 4,096-token long-artifact bound. The response and grader are separate stateless requests. Invocation evidence records only the credential environment-variable name.
...
For the current 90-case suite, 16 of the 95 response requests qualify for the 4,096-token long-artifact bound. Using the larger of `o200k_base` and `cl100k_base`, twice each embedded output maximum for escaping/re-tokenization, and 100 metadata tokens per request gives an uncached input ceiling of 1,751,971 tokens and an output ceiling of 411,648 tokens. At the prices above, the one-run ceiling is **$15.240844**; authorize at least **$16.00** before starting. Recalculate after any prompt, schema, case, routing, output-bound, model, or price change. Stop before any paid request if the recalculated ceiling exceeds the authorized budget. Do not add a paid smoke test, use Batch, enable retries, or resume a completed educational case.
...
Deterministic educational, language, terminology, completeness, text-quality, and consent failures are fail-closed and are never regenerated. A structurally valid `PASS` whose reasoning says behavior required by the criterion is absent, implicit, deferred, or unperformed is conservatively downgraded to `FAIL`; criteria that explicitly require prohibited behavior to be absent remain eligible to pass when exact evidence proves that absence. The report records every downgrade's original verdict and evidence as a grader-protocol adjustment, and the run continues without another paid request. Other malformed-protocol or transport failures may be retried when explicitly enabled, default to one retry, and are capped at two. The report preserves every protocol attempt, retry cause, and conservative adjustment. A later turn cannot erase an earlier selected-turn rejection.

If the OpenAI API returns a non-completed response, the adapter does not parse, expose, or accept partial output. It returns auditable model identity, timing, usage, API identifiers, response status, and `incomplete_details.reason`. The runner records that invocation as `model-protocol-error`, fails the case with absent evidence, skips its grader request, and continues with the remaining cases. The final summary counts model-protocol, grader-protocol, and infrastructure failures separately.
