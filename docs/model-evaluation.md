...
For the current 90-case suite, 19 of the 95 response requests qualify for the 4,096-token long-artifact bound. Using the larger of `o200k_base` and `cl100k_base` for each complete rendered prompt-plus-schema request, twice each embedded output maximum for escaping/re-tokenization, and 100 metadata tokens per request gives an uncached input ceiling of 1,776,572 tokens and an output ceiling of 417,792 tokens. At the prices above, the one-run ceiling is **$15.462128**; obtain at least **$16.00 of additional authorization** before starting a future fresh run. Authorization applies to the full prospective attempt: money already spent on an earlier run is sunk and is excluded rather than treated as remaining credit for a new run. Recalculate after any prompt, schema, case, routing, output-bound, model, or price change. Stop before any paid request if the recalculated ceiling exceeds the new authorization. Do not add a paid smoke test, use Batch, enable retries, or resume a completed educational case.
...
Case-local deterministic failures, non-completed provider responses, and exhausted grader-protocol failures continue through the remaining cases. Whole-run aborts are reserved for global safety or integrity failures: credential-boundary violations, candidate-SHA or release-model drift, stale or corrupt resume evidence, inability to write the report atomically, and unrecoverable adapter transport or infrastructure failure. The report is atomically initialized as `running`, checkpointed through turns, guards, and grading, and finalized as `complete` or `failed`. Resume accepts only a matching Git and run-configuration fingerprint and does not rerun completed educational responses. It preserves immutable prompt packets, raw responses, invocation evidence, artifact hashes and validation, deterministic guards, grader evidence, timestamps, duration, and internally consistent request/failure/skipped/adjustment/usage counts. Instruction bodies are stored once by content hash; prompt packets and journal entries use lossless hash references instead of repeating those bodies.
