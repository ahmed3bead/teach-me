...
def is_general_navigation_clause(text: str) -> bool:
    ...
    english_choice = re.search(
        ...
        r"available|study|learn|build|create|do|work|forget|forgot|remember|achieve|attach|provide)\b|"
        r"\b(?:want|like|hope|need|prefer|use|using|have|available|study|learn|build|create|do|work|"
        r"forget|forgot|remember|achieve|attach|provide)\b.*\b(?:you|your|they|their|learner|students?)\b",
        folded,
    )
    ...
    arabic_choice = re.search(
        ...
        r"ستصور|ستدرس|تحتاج|نسيت|تتذكر|هدفك|عملك|خبرتك|وقتك|جنسيتك|بلدك|سيمتحنك|تسمح)", folded,
    )
...
def assessment_prompt_match(text: str) -> AssessmentMatch | None:
    ...
    for index, original in enumerate(lines):
        ...
        rhetorical_heading = candidate.startswith(("لماذا ", "Why ", "why "))
        if rhetorical_heading and len(following_items) >= 2 and all(item.endswith((".", "!", "؟", "?")) for item in following_items):
            expository_questions.add(candidate)
        unnumbered = re.sub(r"^(?:[0-9٠-٩]+[.)]\s*)", "", candidate)
        if unnumbered.startswith(("ما ", "ماذا ", "هل ")) and any(marker in unnumbered for marker in ("سأ", "أحتاج")):
