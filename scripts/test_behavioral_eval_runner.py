...
def test_registry_and_cases() -> None:
    ...
    journey_source = registry["source.end-to-end-journeys.educator-source-map-approval-lesson.v1"]["result"]["data"]
    assert journey_source["source_id"] == "CURR-PRIMARY-SCI-01"
    ...
    assert Counter(item["classification"] for item in matrix["cases"]) == matrix["category_counts"]
    completed = next(item for item in matrix["cases"] if item["case"] == "conversational-teaching/completed-unit-offer-before-questions")
    assert completed["classification"] == "deterministic-guard false positive"
    assert completed["root_cause"] == "assessment-offer-heuristic"
...
def test_assessment_and_consent() -> None:
    ...
    assert not runner.deterministic_assessment_check("What do you remember about this topic?", none)["passed"]
    assert not runner.deterministic_assessment_check("What do you know about this topic?", none)["passed"]
    assert not runner.deterministic_assessment_check("Which part can you explain?", none)["passed"]
    assert not runner.deterministic_assessment_check("Why is the value 4?\n- A. first option.\n- B. second option.", none)["passed"]
    assert not runner.deterministic_assessment_check("ما مفهوم API؟", none)["passed"]
    assert not runner.deterministic_assessment_check("ما الذي تتذكره عن هذا الموضوع؟", none)["passed"]
    assert not runner.deterministic_assessment_check("Which is correct?\n- A. first option.\n- B. second option.", none)["passed"]
    assert not runner.deterministic_assessment_check("أي جزء تستطيع أن تشرحه؟", none)["passed"]
    good = (
        "Send the book title.",
...
def test_terminology_structure_and_child_semantics() -> None:
    ...
    single_notation = "نقسم التفاحة أولًا إلى أجزاء متساوية. نصفها يكتب 1/2."
    notation_first = "نكتب 1/4 أولًا، ثم نقول إن التفاحة مقسمة إلى أجزاء متساوية."
    assert not runner.deterministic_child_concept_order_check(ordered, ar_prompt)["passed"]
    assert runner.deterministic_child_concept_order_check(single_notation, ar_prompt)["passed"]
...
def test_grader_evidence() -> None:
    ...
    explicitly_hidden_artifact = [{
        "path": "hidden.html",
        "media_type": "text/html",
        "content": (
            "<head><title>hidden title</title></head><template>hidden template</template>"
            "<div hidden>hidden attribute</div><div aria-hidden='true'>hidden aria</div>"
            "<div style='display: none !important'>hidden style</div>"
        ),
    }]
    for hidden_text in ("hidden title", "hidden template", "hidden attribute", "hidden aria", "hidden style"):
        hidden = copy.deepcopy(html_grade); hidden["results"][0]["evidence"].update(artifact_path="hidden.html", quote=hidden_text)
        try: runner.validate_grade(hidden, ["criterion"], transcript, explicitly_hidden_artifact)
        except RuntimeError: pass
        else: raise AssertionError(f"explicitly hidden HTML text passed as learner-visible evidence: {hidden_text}")
